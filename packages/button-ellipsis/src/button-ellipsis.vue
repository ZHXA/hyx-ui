...
<template>
  <div class="hyx-button-ellipsis"   >
      <span   :key="index"  v-for="(item,index) in buttonList" class="hyx-button-ellipsis__button">
          <slot v-if="getComp"  :item="item" ></slot>
          <hyx-tag v-else v-bind="$attrs" round  type="plain" >{{item.name}}</hyx-tag>
      </span>
     <slot  name="number" v-if="count"  :number="'+'+count" class="hyx-button-ellipsis__number" >
       <hyx-tag v-bind="$attrs"  round  type="plain">+{{count}}</hyx-tag>
     </slot>
  </div>
...
export default {
  ...
  methods: {
    ...
    limitShow () {
      ...
      this.$nextTick(() => {
        let buttons = this.$el.querySelector('.hyx-button-ellipsis__button')
