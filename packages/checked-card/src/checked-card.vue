...
<template>
    <div :class="Cls" @click="checkedCard(label)">
      ...
      <!--<div  :class="{'hyx-mask hyx-mask&#45;&#45;disabled':disabled}"></div>-->
...
const prefix = 'hyx-checked-card'
const classnames = require('classnames')
export default {
  name: 'hyx-checked-card',
  data () {
    return {
      prefix
    }
  ...
  computed: {
    ...
    checkedCls () {
      const className = classnames('hyx-icon', 'icon-gou', {
        [`hyx-icon--${this.color || 'primary'}`]: this.isChecked
      })
      return this.isChecked && className
    },
    getModelType () {
      const cls = classnames('hyx-mask', {
        'hyx-mask--disabled': this.disabled,
        [`hyx-mask--${this.modelType}`]: this.isChecked
      })
      return cls
    }
