<template>
  <div class="hyx-text-ellipsis">
    <slot name="before" class="hyx-text-ellipsis__before"></slot>
    <span :style="textStyle" :class="textClass" @click="textClick"  ></span>
    <span  class="hyx-text-ellipsis__limit-text" :key="keyIndex">{{text}}</span>
    <span class="hyx-text-ellipsis__more"  v-show='oversize' @click="textClick" >{{more}}<slot name="more"></slot></span>
    <slot name="after" class="hyx-text-ellipsis__after"></slot>
  </div>
...
export default {
  ...
  methods: {
    ...
    init () {
      this.oversize = false
      this.keyIndex += 1
      let more = this.$el.querySelector('.hyx-text-ellipsis__more')
    ...
    limitShow () {
      this.$nextTick(() => {
        let textDom = this.$el.querySelector('.hyx-text-ellipsis__limit-text')
        let title = this.$el
        let more = this.$el.querySelector('.hyx-text-ellipsis__more')
